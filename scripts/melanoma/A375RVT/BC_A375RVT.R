...
A375RVT_data <- readRDS(file="/home/lpgonzalezm/TFM/beyondcell/prueba_melanoma/A375RVT/A375RVT.rds")
bc_recomputed <- readRDS("/home/lpgonzalezm/TFM/beyondcell/prueba_melanoma/A375RVT/bc_A375RVT.rds")

SSc <- GetCollection(SSc, n.genes = 250, mode = c("up", "down"))
bc <- bcScore(A375RVT_data, SSc, expr.thres = 0.1) 
...
saveRDS(bc_recomputed, file = "/home/lpgonzalezm/TFM/beyondcell/prueba_melanoma/A375RVT/bc_A375RVT.rds")
...
bc_recomputed <- bcRegressOut(bc_recomputed, vars.to.regress = "nCount_RNA", k.neighbors = 40)
...
bcClusters(bc_recomputed, UMAP = "beyondcell", idents = "bc_clusters_res.1", label.size = 5, pt.size = 1.8)
A375RVT_data <- AddMetaData(object = A375RVT_data, metadata = bc_recomputed@meta.data)
...
DimPlot(
  ...
  label = TRUE,
  label.size = 5,
  group.by = c("bc_clusters_res.0.6", "RNA_snn_res.0.4"),
...
ggsave(filename = "bc_recomputed_drugs0_A375RVT.jpg", path = "/home/lpgonzalezm/TFM/beyondcell/prueba_melanoma/A375RVT/plots_reg/bc_recomputed/", width = 16, height = 9)
bc4Squares(bc_recomputed, idents = "bc_clusters_res.0.6", lvl = 1 , top = 5)
ggsave(filename = "bc_recomputed_drugs1_A375RVT.jpg", path = "/home/lpgonzalezm/TFM/beyondcell/prueba_melanoma/A375RVT/plots_reg/bc_recomputed/", width = 16, height = 9)
bc4Squares(bc_recomputed, idents = "bc_clusters_res.0.6", lvl = 2 , top = 5)
ggsave(filename = "bc_recomputed_drugs2_A375RVT.jpg", path = "/home/lpgonzalezm/TFM/beyondcell/prueba_melanoma/A375RVT/plots_reg/bc_recomputed/", width = 16, height = 9)
bc4Squares(bc_recomputed, idents = "bc_clusters_res.0.6", lvl = 3 , top = 5)
ggsave(filename = "bc_recomputed_drugs3_A375RVT.jpg", path = "/home/lpgonzalezm/TFM/beyondcell/prueba_melanoma/A375RVT/plots_reg/bc_recomputed/", width = 16, height = 9)
...
A375RVT_data <- AddMetaData(A375RVT_data, metadata =bc_recomputed@meta.data)

###########VEMURAFENIB#############
SSc <- GetCollection(SSc, n.genes = 250, mode = c("up", "down"), )
vemurafenib_gs <- GenerateGenesets(x = "/home/lpgonzalezm/Downloads/vemurafenib_signatures.gmt", )

bc <- bcScore(A375RVT_data, SSc, expr.thres = 0.05) 
bc_filtered <- bcSubset(bc, nan.cells = 0.95)
bc_filtered@normalized[is.na(bc_filtered@normalized)] <- 0                                                                                                                                                                       
bc_recomputed <- bcRecompute(bc_filtered, slot = "normalized") 

bc_vemurafenib <- bcScore(A375RVT_data, vemurafenib_gs, expr.thres = 0.05) 
...
saveRDS(bc_recomputed_merged, file = "/home/lpgonzalezm/TFM/beyondcell/prueba_melanoma/A375RVT/bc_merged_A375RVT.rds")
